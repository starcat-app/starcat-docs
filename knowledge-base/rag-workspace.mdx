...
- Long thinking stays usable; expand the full text when it finishes. Query-planning substeps and the plan pane appear only after planning actually completes  
...
The gear opens settings for the inference backend, prompts, and retrieval strategy. Prompt editing is covered in [AI configuration](/ai-features/configuration).

## Direct: local CLI inference

Knowledge-base RAG can use local **Codex CLI** or **Claude Code CLI** and reuse their existing login. Workspace settings detect install, version, and path, and show setup help when missing. The Mac App Store build does not include this path.
...
Generated [Insights](/main-interface/insights) (library aggregates and per-repo) can attach as removable read-only context for answers. Custom prompts that omit insight placeholders will not inject them silently.
