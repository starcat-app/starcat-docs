...
配对之后在终端搜 Stars、改标签 / 状态 / 笔记，或把 `starcat mcp` 交给外部 Agent。写命令默认不落库，必须 `--apply`。App 侧写权限关掉时，即使 `--apply` 也会失败。
...
CLI 不打开 SQLite。见 [MCP 设置](/zh-Hans/settings/mcp)、[Skill](/zh-Hans/cli/skill)。
