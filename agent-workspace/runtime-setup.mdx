...
- If Codex shows **Codex CLI**, automatic detection found the installed CLI. The
  status intentionally does not claim that authentication succeeded;
  `codex login status` remains the source of truth.
- To use a standalone package, choose its Runtime directory. Starcat accepts
  binaries directly in that directory or in its `bin/` subdirectory. The package
  must contain executable `codex-app-server` (or `codex`) and
  `codex-code-mode-host` files.
- **Use Automatic Detection** clears a manually configured path and searches `PATH`,
  Homebrew, `~/.local/bin`, `~/.npm-global/bin`, and `~/.bun/bin` again.
