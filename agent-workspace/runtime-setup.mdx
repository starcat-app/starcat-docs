...
### 2. Let Starcat find the executable

Open **Settings → Integrations → Agent Runtime**.

- If Codex shows **Executable found**, the path is ready. The status intentionally
  does not claim that authentication succeeded; `codex login status` is the source
  of truth.
- If Codex is not found, click **Choose Executable** and select the path printed by
  `command -v codex`.
- **Use Automatic Detection** clears a manually selected path and searches `PATH`,
...
works but Starcat cannot find Codex, select the exact `command -v codex` path in
Settings.
